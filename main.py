...
def setup_gpio(callback):
    ...
    for pin in SWITCH_PINS:
        try:
            ...
            GPIO.add_event_detect(pin, GPIO.RISING, callback=callback, bouncetime=1000)
        except Exception as e:
            print(f"Error setting up GPIO pin: {pin}")
    try:
        GPIO.setup(PIN_RESET, GPIO.IN, pull_up_down=GPIO.PUD_UP)
        GPIO.add_event_detect(PIN_RESET, GPIO.RISING, callback=callback, bouncetime=100) # check time later
    except Exception as e:
