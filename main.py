...
class SkeeBall:
    ...
    def _draw_playing(self):
        ...
        for i in range(MAX_BALLS):
            ...
            if i % 2 == 0:
                pygame.draw.circle(scr, BALL_LAB_COL, (bx + (i//2) * (ball_r * 2 + ball_spacing), ball_y), ball_r+3 + (PI*3))
                pygame.draw.circle(scr, col, (bx + (i//2) * (ball_r * 2 + ball_spacing), ball_y), ball_r)
            else: 
                pygame.draw.circle(scr, BALL_LAB_COL, (diff + bx + (i//2) * (ball_r * 2 + ball_spacing), ball_y_2), ball_r+3+ (PI*3))
